...
**Question for Brian before proceeding:** The upgrade instructions say "confirm text-readability library is used for all four metrics." Currently the formulas are computed manually with our custom sentence count — this is what fixed the 10+ grade-level discrepancy vs. Readable.com. If Change Set C requires switching back to `rs.fleschKincaidGrade()` etc., scores will regress significantly. **Please confirm whether "use the library" means using its syllable/word counters (which we do) or also using its sentence splitter (which we intentionally bypass for accuracy).** I will not change this until confirmed.
